...
def app(state):
    ...
    state_reg = SessionState.get(train_start=False,
                                save_btn=False,
                                reg_model=None,
                                graph=None,
                                prep_data=True)
    ...
    ml_type = 'regression'
    st.title(" Neural Networks - Regression :chart_with_upwards_trend:")
    ...
    if prep_data :
        state_reg.prep_data = True
    else :
        state_reg.prep_data = False

    train_start = st.sidebar.button("Train")  
    if train_start :
        state_reg.train_start = True 
    else:
        state_reg.train_start = False
    
    if (state.df is not None) or (state_reg.train_start == True):
        
        with main_canvas.beta_container():
            st.markdown("### Loss/Metric Vs Epoch")
            state_reg.graph = st.line_chart(pd.DataFrame())
        ...
        if state_reg.train_start:
            if state.target_choice is not None:
                ...
                callbacks = LivePlot(2,train_loss='loss',train_metric='mean_absolute_error',graph=state_reg.graph)
                nn = DNN_Model(type=ml_type)
                state_reg.kwargs = validate_hparams(hparams)

                if state_reg.kwargs != None:
                    m1 = nn.create_model(X_train.shape[1],1,**state_reg.kwargs)
                else:
                    m1 = nn.create_model(X_train.shape[1],1)
                state_reg.reg_model = m1
                train_results = nn.train_model(m1,X_train,y_train,['mean_absolute_error'],['mean_absolute_error'],callbacks=[callbacks],ret_result=True)   
                test_results = nn.test_model(X_test,y_test)
                score_df = score_df.append(pd.DataFrame({'Train Score':train_results[1], 'Test Score':test_results[1]},index=['Mean Absolute Error']))
                with score_cont.beta_container():
                    score.add_rows(score_df)
                if len(score_df) > 0:
                    train_complete = True
                    hist_df = pd.DataFrame(nn.history.history)
                    plot_all_charts(state_reg.graph,hist_df)
                else:
        ...
        with save_model_col:
            ...
            save_info = st.empty()
            if save_btn :
                state_reg.save_btn = True
                if train_complete:
                    with save_info.beta_container():
                        st.info(f"Saving Model {state_reg.reg_model}")

                    plot_all_charts(state_reg.graph,hist_df)
                    with score_cont.beta_container():
                        score = st.table(score_df)
                    model_name = save_model(state_reg.reg_model,state.data_file_name_no_ext,ext=True,type=ml_type)
                ...
                elif state_reg.train_start == False:
                    with save_info.beta_container():
